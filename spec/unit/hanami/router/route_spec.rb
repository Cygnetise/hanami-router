...
RSpec.describe Hanami2::Router::Route do
  describe "#inspect_to" do
    context "when it's a String" do
      it "returns it unmodified" do
        route = Hanami2::Router::Route.new(http_method: "GET", path: "/", to: "/")
    ...
    context "when it's a Class" do
      it "returns (class) when it's anonymous" do
        route = Hanami2::Router::Route.new(http_method: "GET", path: "/", to: Class.new)

        expect(route.inspect_to).to eq("(class)")
      end

      it "returns the class name when it's named" do
        route = Hanami2::Router::Route.new(http_method: "GET", path: "/", to: described_class)

        expect(route.inspect_to).to eq("Hanami2::Router::Route")
      end
    end

    context "when it's a Hanami2::Router::Block" do
      it "returns (block)" do
        route = Hanami2::Router::Route.new(http_method: "GET", path: "/", to: Hanami2::Router::Block.new(-> {}, :development))

        expect(route.inspect_to).to eq("(block)")
      end
    end

    context "when it's a Hanami2::Router::Redirect" do
      it "returns formatted destination and HTTP code" do
        route = Hanami2::Router::Route.new(http_method: "GET", path: "/", to: Hanami2::Router::Redirect.new("redirect", 301, "/"))
    ...
    context "when it's an instance" do
      it "returns (class) when its class is anonymous" do
        route = Hanami2::Router::Route.new(http_method: "GET", path: "/", to: Class.new.new)

        expect(route.inspect_to).to eq("(class)")
      end

      it "returns its class name when its class is named" do
        route = Hanami2::Router::Route.new(http_method: "GET", path: "/", to: Object.new)
  ...
  context "#inspect_constraints" do
    ...
    it "returns formatted constraints when there're some" do
      route = Hanami2::Router::Route.new(http_method: "GET", path: "/", to: "/", constraints: {id: /\d+/})
  ...
  context "#inspect_as" do
    ...
    it "returns as string when there's an as" do
      route = Hanami2::Router::Route.new(http_method: "GET", path: "/", to: "/", as: :root)
