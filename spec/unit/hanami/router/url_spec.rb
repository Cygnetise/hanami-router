...
RSpec.describe Hanami2::Router do
  let(:router) do
    e = endpoint
    Hanami2::Router.new(base_url: base_url) do
  ...
  describe "#url" do
    ...
    it "raises error when variables aren't satisfied" do
      expect { router.url(:variables) }.to raise_error(Hanami2::Router::InvalidRouteExpansionError, "No route could be generated for `:variables': cannot expand with keys [], possible expansions: [:id]")
    end
    ...
    xit "raises error when insufficient params are passed" do
      expect { router.url(nil) }.to raise_error(Hanami2::Router::InvalidRouteExpansionError, "No route could be generated for nil - please check given arguments")
    end
